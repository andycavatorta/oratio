...
def network_message_handler(msg):
    ...
    if topic == "voice_3":
        ...
        if (payload[1] > 0.2):
            ...
            c.set_levels(0, 255 if payload[1] < 0.5 else (180 * payload[1]))
            c.set_levels(1, 255 if payload[3] < 0.5 else (180 * payload[1]))
            c.set_levels(2, 255 if payload[5] < 0.5 else (180 * payload[1]))
        else:
            c.send_freq(0, 0)
            c.send_freq(1, 0)
            c.send_freq(2, 0)
